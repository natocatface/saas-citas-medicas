...
@section('subtitle', 'Cuentas y roles del sistema')
...
<div class="space-y-5">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center gap-3">
        <form method="GET" class="flex flex-col sm:flex-row gap-3 flex-1">
            ...
            <select name="rol" onchange="this.form.submit()" class="rounded-lg border-slate-300 text-sm focus:border-brand-500 focus:ring-brand-500 sm:w-48">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/60">
                            <td class="px-5 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-brand-400 to-brand-600 text-white text-xs font-bold flex items-center justify-center shrink-0">
                                        {{ $u->iniciales() }}
                                    </div>
                                    <div>
                            ...
                            <td class="px-5 py-3">
                                <span class="px-2.5 py-1 rounded-full text-[11px] font-semibold {{ $rolBadge[$u->rol] ?? 'bg-slate-100 text-slate-600' }}">
                                    {{ $roles[$u->rol] ?? ucfirst($u->rol) }}
                                </span>
                            </td>
                            ...
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('usuarios.edit', $u) }}" class="px-2.5 py-1.5 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-brand-600 text-xs font-medium">Editar</a>
                                    @if($u->id !== auth()->id())
                                        @include('partials.boton-eliminar', ['action' => route('usuarios.destroy', $u), 'nombre' => $u->name])
                                    @else
                    ...
                    @empty
                        <tr><td colspan="5" class="px-5 py-10 text-center text-slate-400">No se encontraron usuarios.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
